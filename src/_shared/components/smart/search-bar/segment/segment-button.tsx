...
>(function SegmentButton(
  ...
  return (
    <Button
      ...
      className={cn(
        [ 'h-15 min-w-60 rounded-xl', config.hideSeparatorIds.map((id) => `hide-${id}`) ],
        displayStatus === 'open' && [
          'aria-expanded:bg-white border-0 shadow-md',
          config.onOpenBorder,
        ],
    ...
    >
      <div className='text-left w-full px-4 text-xs flex flex-col gap-1 font-normal'>
        {children}
      </div>
    </Button>
  );
